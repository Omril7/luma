...
export function EmailServicesPage() {
  ...
  const loadSettings = useCallback(async () => {
    ...
    setLoadError(null)
    try {
      const { settings } = await api.get<{ settings: EmailSettings }>(
        '/api/admin/email-settings',
        token
      )
      setForm({
        fromAddress: settings.fromAddress ?? '',
        fromName_he: settings.fromName_he ?? '',
        fromName_en: settings.fromName_en ?? '',
  ...
  async function handleSaveSender() {
    ...
    setSenderError(null)
    try {
      await api.put<{ settings: EmailSettings }>(
        '/api/admin/email-settings',
        {
  ...
  return (
    <div className="space-y-5 max-w-2xl">
      ...
      <div>
        ...
        <p className="text-sm text-text-muted mt-0.5">הגדרות שולח למיילים שנשלחים מהאתר</p>
      </div>
