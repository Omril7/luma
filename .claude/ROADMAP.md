...
- [ ] `src/shared/schemas/` (Zod: order, contact, newsletter, price-request, admin login, coupon)
...
- **Acceptance:** typing custom dimensions updates the price instantly and matches the server. *(Depends on M0.2, M1.5.)*
...
- **Acceptance:** Preview + Production deployments serve the storefront and `/api`; images via Cloudinary; emails via Nodemailer SMTP. *(See `docs/10-devops.md`.)*
