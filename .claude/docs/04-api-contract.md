...
| Method | Path                                | Purpose                                                                                                                                                                                                                                                                                                                                                                   |
| ------ | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/api/products`                     | List products. Query: `category`, `featured`, `active`, `sort` (`recommended`/`price_asc`/`price_desc`/`newest`/`name_he`/`name_en`), `page`, `pageSize`. `recommended` = `sortOrder` asc then `createdAt` desc (the admin's drag order). `/shop` itself no longer calls this — it fetches the whole catalog once (`getShopCatalogCached`) and filters/sorts client-side. |
...
| —      | `/api/admin/reviews`          | List (filter `status`, `scope`), approve/reject, edit, `featuredOnHome` toggle, `+ New review`.                                                                       |
| POST   | `/api/admin/reviews/reorder`  | Bulk drag-reorder of the homepage-featured reviews. Body `{ ids: string[] }` → `sortOrder` = index. Drives `getFeaturedHomeReviews` (homepage `TestimonialsSection`). |
