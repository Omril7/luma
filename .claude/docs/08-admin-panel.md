...
> email settings, **and order management** (`/admin/orders` — see `09-payments.md`). The
> companion **luma-manager** app connects to the same Supabase database but is a separate
> business/financial manager (income, expenses, VAT, pricing) — it does not do order management.
...
| Orders         | `/admin/orders`         | 1 (planned)   | List/detail, fulfillment status, payment status, Morning refunds + resend — see `09-payments.md`.                          |
