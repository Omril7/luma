---
description: Build a full vertical slice of a feature — data → API → store → UI page → i18n keys — across all workspaces.
...
1. **Data** (`.claude/docs/02-data-models.md`): add/adjust Prisma models + a migration if
   ...
   says so.
2. **Shared:** Zod schemas + shared types in `shared/`. Any pricing/calculation logic goes in
   `shared/` (pure, tested), never duplicated client/server.
3. **API** (`.claude/docs/04-api-contract.md`): endpoints via the `/new-endpoint` pattern
   (route + controller + service + tests, correct envelope/auth/rate-limit).
4. **Client state** (`.claude/docs/05-frontend.md`): a Zustand store slice and/or `lib/api.ts`
   methods.
5. **UI:** components under `features/<name>/` + a page if it's a route. Mobile-first,
   RTL-safe (logical properties), themed via CSS variables.
